// Verify every level: solve it, replay the solution through game commit-logic, assert isWin.
...
function solve(start){ if(isWin(start))return {moves:0,path:[]};
  ...
  const best=new Map(); best.set(key(start),0); push(heur(start),{tb:start,g:0,path:[]}); let nodes=0;
  while(heap.length){ const [,cur]=pop(); const {tb,g,path}=cur; const ck=key(tb); if(g>(best.get(ck)??1e9))continue;
    if(isWin(tb))return {moves:g,path}; if(++nodes>2000000)return null;
...
for(let i=0;i<LEVELS.length;i++){
  const lvl=LEVELS[i];
  // sanity: each color appears exactly CAP times
  const counts={}; lvl.tubes.flat().forEach(c=>counts[c]=(counts[c]||0)+1);
  const badCount=Object.entries(counts).filter(([,n])=>n%CAP!==0);
  const res=solve(lvl.tubes.map(t=>t.slice()));
  let replayWin=false, replayPours=0;
  if(res){ let st=lvl.tubes.map(t=>t.slice()); for(const [a,b] of res.path){ st=doPour(st,a,b); replayPours++; } replayWin=isWin(st); }
  const optimalMatch = res && res.moves===lvl.optimal;
  const ok = res && replayWin && optimalMatch && badCount.length===0;
  if(!ok) allOk=false;
  console.log(`L${String(i+1).padStart(2)} ${ok?'PASS':'FAIL'}  tubes=${lvl.tubes.length} solverMoves=${res?res.moves:'X'} declaredOptimal=${lvl.optimal} replayWin=${replayWin} colorCountsOK=${badCount.length===0}`);
}
